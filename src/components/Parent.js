...
//               listStyle: "none",
//             }}
//             onClick={() => props.setState(data.children?.length ? true : false)}
//           >
//             <div>{data.first_name}</div>
//             <div>{data.title}</div>
//             {/* <div>{data.EmployeeLevel}</div> */}
//             <div>{data.children?.length ? data.children.length : ""}</div>
//           </li>
//           {data.children?.length ? <FlexBox data={data.children} /> : ""}
//         </div>
//       ))}
//     </div>
//   );
...
//             <Chip
//               avatar={<Avatar sx={{ width: 40, height: 40, filter: state ? "" : "grayscale(100%)" }} src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80">{data.first_name.slice(0, 1)}</Avatar>}
//               label={<div><div style={{ color: state ? "" : "gray" }}>{data?.first_name + " " + data?.last_name}</div><div style={{ fontSize: "12px", paddingLeft: "5px", color: state ? "" : "black" }}>{data.title}</div></div>}
...
//         <div style={{ border: data.children && state ? "1px solid blue" : "1px solid gray", width: "20px" }}></div>
//         {/* avatr containg count of children */}
//         <Avatar
//           variant="rounded"
//           sx={{ width: 24, height: 24, bgcolor: data.children.length? "#3c71ff":"gray", fontSize: "0.8rem" }}
//         >
...
//generic component for both employee and department tree
import React from "react";
import { useState } from "react";
import data2 from "../data2.json";
import Avatar from "@mui/material/Avatar";
...
function Parent({ data }) {
  ...
  return (
    ...
    <div style={{ marginTop: "15px" }} className="tree">
      <div style={{ display: "flex", alignItems: "center" }}>
        <Tooltip title={data?.name || data?.first_name}>
          {/* node elements */}
          <Stack direction="row" spacing={1} width={280}>
            <Chip
        ...
        {/* horizontal Line from node element to avatar showing no. of child  */}
        {(data.children && data.children.length>0)&&<div style={{ border: data.children && state ? "1px solid blue" : "1px solid gray", width: "20px" }}></div>}
        {/* avatr containg count of children */}
